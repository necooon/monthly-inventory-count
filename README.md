...
| 発注 | 足りないアイテムについて、商品を選ぶか、購入先を入力して確定する。 |
...
現行の実装は、発注と買い物・受け取りを分け、商品と購入履歴を設定で扱います。クラウドに `products` / `purchase_history` テーブルがない場合は端末内に保存します。スキーマ追加は [`supabase/setup.sql`](supabase/setup.sql) の末尾を SQL Editor で実行してください。
