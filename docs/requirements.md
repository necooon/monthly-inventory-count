...
店やネットで買う「実商品」と、家の在庫として数える「アイテム」を分ける。買ったことは履歴で分かり、在庫数は次の Check で合わせる。
...
購入先はアイテムにも残す。発注で個別商品を選ばず購入先を入力するときの候補になる。
...
ひとつのアイテムに複数の個別商品が紐づく（一対多）。同じ個別商品を別アイテムでは使い回さない。所属アイテムは常に1つ。

付け外しはアイテム編集からも商品編集からもできる。

### 削除

個別商品は履歴の根拠になるため削除しない。非表示が必要なら Later。

## 3. 画面

| 画面 | 役割 |
|------|------|
| Check | 棚卸。場所・頻度で数量を入れる。在庫の正を更新する。 |
| 発注 | 要発注アイテムに対し、個別商品の選択または購入先の入力で確定する。 |
| 買い物・受け取り | 発注確定後の実行。店舗は買い物、ネットは受け取り。発注とは分ける。 |
| 設定 | アイテム、個別商品、履歴、既存マスタ（頻度・場所・カテゴリ・購入先）。 |
...
要発注アイテムを並べ、行ごとに次のいずれかで確定する。両方とも空のまま完了にはできない。

- **個別商品を選ぶ**（そのアイテムに紐づく商品から）。行き先はその商品の購入先になる。
- **購入先を入力する**（マスタから選ぶ、または新規入力）。個別商品は選ばない。アイテム側の購入先は候補として残す。
...
- 通知
- ログイン・複数世帯
- バーコードのスキャン登録 UI（バーコードは個別商品の属性として持つ）
