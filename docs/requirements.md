...
1. **LOHACO 選択** — 要発注アイテムを購入先ごとにまとめ、LOHACO を先頭にする。チェックしたものをネットで注文するか、リストに追加できる。アイテムまたは紐づく商品の購入先に LOHACO があるものは最初からチェック済み。商品が LOHACO 向けに1件だけ紐づいていればそれを付け、複数または無しなら購入先だけで確定する。
