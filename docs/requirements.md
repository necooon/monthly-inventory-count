...
1. **LOHACO 選択** — 要発注アイテムにチェックを入れ、選んだものを LOHACO で一括確定する。アイテムまたは紐づく商品の購入先に LOHACO があるものは最初からチェック済み。商品が LOHACO 向けに1件だけ紐づいていればそれを付け、複数または無しなら購入先だけで確定する。スキップすると店舗などの発注へ進む。
