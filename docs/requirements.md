...
| 発注 | まず LOHACO で買うアイテムを選んで一括確定する。残った要発注アイテムは、商品の選択または購入先の入力で確定する。 |
...
1. **LOHACO 選択** — 要発注アイテムにチェックを入れ、選んだものを LOHACO で一括確定する。商品が LOHACO 向けに1件だけ紐づいていればそれを付け、複数または無しなら購入先だけで確定する。スキップすると店舗などの発注へ進む。
...
Check / 発注 / 買い物（受け取り切替）/ 設定。商品と履歴は設定。購入完了では在庫数を増やさない。クラウド用の `products` と `purchase_history` は [`supabase/setup.sql`](../supabase/setup.sql) 末尾。テーブル未作成時は端末の localStorage に持つ。
